...
def generate_html_report(models_results, plots_data, target_col):
    ...
    model_info = {
        ...
        'Support Vector Regressor': 'Finds optimal boundaries in high-dimensional space.',
        'XGBoost': 'Gradient boosting algorithm with regularization to prevent overfitting.',
        'Neural Network': 'Learns complex patterns through interconnected layers of neurons.'
    }
    ...
    metric_info = {
        ...
        'r2': 'R² Score: Proportion of variance in target explained by the model.'
    }
    ...
    <head>
        ...
        <style>
            ...
            .model-info {{ background-color: #f9f9f9; padding: 15px; border-radius: 5px; margin-bottom: 20px; }}
            .metric-help {{ font-size: 0.9em; color: #666; margin-top: 5px; }}
        </style>
    </head>
    <body>
    ...
    for i, (model_name, results) in enumerate(sorted_models, 1):
        ...
        <div class="model-section">
            ...
            <table class="metrics-table">
                ...
                <tr>
                    ...
                    <td class="metric-help">{metric_info['mse']}</td>
                </tr>
                <tr>
                    <td>MAE</td>
                    ...
                    <td class="metric-help">{metric_info['mae']}</td>
                </tr>
                <tr>
                    <td>R²</td>
                    ...
                    <td class="metric-help">{metric_info['r2']}</td>
                </tr>
                <tr>
                    <td>Training Time (s)</td>
                    <td colspan="3">{results['training_time']:.2f}</td>
                    <td class="metric-help">Time taken to train the model</td>
                </tr>
            </table>
        ...
        for dataset in ['train', 'test', 'val']:
            if f"{model_name}_{dataset}" in plots_data:
                ...
                has_feature_importance = any('Random Forest' in model_name or 
                                           'XGBoost' in model_name or
                                           'Decision Tree' in model_name for model_name in models_results.keys())
                ...
                <div class="plot">
                    ...
                    <div class="metric-help">
                        <p><strong>Top Left:</strong> Actual vs Predicted values (ideal points fall on dashed line)</p>
                        <p><strong>Top Right:</strong> Residual distribution (should be centered at zero)</p>
                        <p><strong>Bottom Left:</strong> Time series comparison of first 100 samples</p>
                        {f'<p><strong>Bottom Right:</strong> Feature importance scores</p>' if has_feature_importance else ''}
                    </div>
                </div>
                """
